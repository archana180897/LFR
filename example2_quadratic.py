# Code to accompany Chapter 5 of PhD Thesis of Archana Arya.

# Example 2:
# This code is for the Quadratic finite element spatial discretization of
# this problem with sixth order implicit leapfrog scheme.
...
# 6th order Implicit Leap-Frog
print("\t\tusing 6th order implicit Leap Frog with time step of %1.4f"%dt)

# LF-6 Linear Operator Computation for use in Iterative Solution
Np = N0 + N1
NE = 2*N1 + 2*N2
NH = 3*N2
...
def matvec(v):
    ...
    z3 = solverM00.solve(M00_g, S01_g * z2)
    z4 = solverM11.solve(M11_g, S01_g.T * z3)
    w1 = B1 * v1 + B2 * (-v2 + dt**2/12 * z2 -  dt**4/120 * z4)

    # Compute the second block vector of the matrix-vector product
    z5 = solverM11.solve(M11_g, S01_g.T * v1)
    z6 = solverM00.solve(M00_g, S01_g * z5)
    z7 = solverM11.solve(M11_g, S01_g.T * z6)
    z8 = solverM00.solve(M00_g, S01_g * z7)
    z9 = solverM11.solve(M11_g, S12_g * v3)
    z10 = solverM22.solve(M22_g, S12_g.T * z9)
    z11 = solverM11.solve(M11_g, S12_g * z10)
    z12 = solverM22.solve(M22_g, S12_g.T * z11)
    w2 = (B4 * (v1 - dt**2/12 * z6 + dt**4/120 * z8) + B5 * v2 +
            B6 * (v3 + dt**2/(12*mu*epsilon) * z10 + dt**4/(120*mu**2*epsilon**2) * z12))

    # Compute the third block vector of the matrix-vector product
    z13 = solverM22.solve(M22_g, S12_g.T * v2)
    z14 = solverM11.solve(M11_g, S12_g * z13)
    z15 = solverM22.solve(M22_g, S12_g.T * z14)
    z16 = solverM11.solve(M11_g, S12_g * z15)
    w3 = 1/2*S12_g.T * (v2 + dt**2/(12*mu*epsilon) * z14 + dt**4/(120*mu**2*epsilon**2) * z16) + mu/dt*M22_g * v3
...
A01_g_approx = M11_g_inv_approx * S01_g.T * M00_g_inv_approx * S01_g
A12_g_approx = M22_g_inv_approx * S12_g.T * M11_g_inv_approx * S12_g
X01_g_approx = S01_g * A01_g_approx
X12_g_approx = S12_g * A12_g_approx
Y01_g_approx = S01_g * A01_g_approx * A01_g_approx
Y12_g_approx = S12_g * A12_g_approx * A12_g_approx

S_LHS_precond = sprs.bmat([[1/dt*M00_g, epsilon/2*(-S01_g + dt**2/12*X01_g_approx - dt**4/120*Y01_g_approx), None], 
                            [-1/2*(S01_g.T - dt**2/12*X01_g_approx.T + dt**4/120*Y01_g_approx.T), -epsilon/dt*M11_g, 
                                1/2*(S12_g + dt**2/(12*mu*epsilon)*X12_g_approx + dt**4/(120*mu**2*epsilon**2)*Y12_g_approx)],
                            [None, 1/2*(S12_g.T + dt**2/(12*mu*epsilon)*X12_g_approx.T + 
                            dt**4/(120*mu**2*epsilon**2)*Y12_g_approx.T), mu/dt*M22_g]], format='csr')
...
# A01_g = M11_g_inv * S01_g.T * M00_g_inv * S01_g
# A12_g = M22_g_inv * S12_g.T * M11_g_inv * S12_g
# X01_g = S01_g * A01_g
# X12_g = S12_g * A12_g
# Y01_g = S01_g * A01_g * A01_g
# Y12_g = S12_g * A12_g * A12_g

# # Setup the linear system for the solution of p, E and H
# S_LHS = sprs.bmat([[1/dt*M00_g, epsilon/2*(-S01_g + dt**2/12*X01_g - dt**4/120*Y01_g), None], 
#                 [1/2*(S01_g.T - dt**2/12*X01_g.T + dt**4/120*Y01_g.T), epsilon/dt*M11_g, 
#                     -1/2*(S12_g + dt**2/(12*mu*epsilon)*X12_g + dt**4/(120*mu**2*epsilon**2)*Y12_g)],
#                 [None, 1/2*(S12_g.T + dt**2/(12*mu*epsilon)*X12_g.T + dt**4/(120*mu**2*epsilon**2)*Y12_g.T), mu/dt*M22_g]], 
#                 format='csr')
...
E3 = solverM00.solve(M00_g, S01_g * E2)
E4 = solverM11.solve(M11_g, S01_g.T * E3)
bp0_RHS = 2/dt*M00_g * p[0] + epsilon/4 * S01_g * (E[0] - dt**2/48 * E2 + dt**4/1920 * E4)

p1 = solverM11.solve(M11_g, S01_g.T * p[0])
p2 = solverM00.solve(M00_g, S01_g * p1)
p3 = solverM11.solve(M11_g, S01_g.T * p2)
p4 = solverM00.solve(M00_g, S01_g * p3)
H1 = solverM11.solve(M11_g, S12_g * H[0])
H2 = solverM22.solve(M22_g, S12_g.T * H1)
H3 = solverM11.solve(M11_g, S12_g * H2)
H4 = solverM22.solve(M22_g, S12_g.T * H3)
bE0_RHS = (1/4 * S01_g.T * (p[0] - dt**2/48 * p2 + dt**4/1920 * p4) - 2*epsilon/dt * M11_g * E[0] -
            1/2 * S12_g * (H[0] + dt**2/(48*mu*epsilon) * H2 + dt**4/(1920*mu**2*epsilon**2) * H4))

E1 = solverM22.solve(M22_g, S12_g.T * E[0])
E2 = solverM11.solve(M11_g, S12_g * E1)
E3 = solverM22.solve(M22_g, S12_g.T * E2)
E4 = solverM11.solve(M11_g, S12_g * E3)
bH0_RHS = -1/4 * S12_g.T * (E[0] + dt**2/(48*mu*epsilon) * E2 + dt**4/(1920*mu**2*epsilon**2) * E4) + mu/dt*M22_g * H[0]
...
# b0_RHS_direct = sprs.bmat([[2/dt*M00_g, epsilon/4*(S01_g - dt**2/48*X01_g + dt**4/1920*Y01_g), None], 
#                            [1/4*(-S01_g.T + dt**2/48*X01_g.T - dt**4/1920*Y01_g.T), 2*epsilon/dt*M11_g, 
#                             1/2*(S12_g + dt**2/(48*mu*epsilon)*X12_g + dt**4/(1920*mu**2*epsilon**2)*Y12_g)],
#                            [None, -1/4*(S12_g.T + dt**2/(48*mu*epsilon)*X12_g.T + dt**4/(1920*mu**2*epsilon**2)*Y12_g.T), 
#                             mu/dt*M22_g]], format='csr') * np.concatenate((p[0], E[0], H[0]))

# # Impose boundary conditions on the right hand side at this time step
# b0_RHS_direct[boundary_vertex_indices] = p_bc[:N0b]
# b0_RHS_direct[N0 + boundary_edge_indices] = p_bc[N0b:]
# for i in range(2):
#     b0_RHS_direct[N0 + N1 + 2*boundary_left_edges + i] = E_left_boundary
#     b0_RHS_direct[N0 + N1 + 2*boundary_right_edges + i] = E_right_boundary
#     b0_RHS_direct[N0 + N1 + 2*boundary_top_edges + i] = E_top_boundary
#     b0_RHS_direct[N0 + N1 + 2*boundary_bottom_edges + i] = E_bottom_boundary
...
def matvec_0(v):
    ...
    z3 = solverM00.solve(M00_g, S01_g * z2)
    z4 = solverM11.solve(M11_g, S01_g.T * z3)
    w1 = B1_0 * v1 + B2_0 * (-v2 + dt**2/48 * z2 -  dt**4/1920 * z4)
    ...
    z11 = solverM11.solve(M11_g, S12_g * z10)
    z12 = solverM22.solve(M22_g, S12_g.T * z11)
    w2 = (B4_0 * (v1 - dt**2/48 * z6 + dt**4/1920 * z8) + B5_0 * v2 +
            B6_0 * (v3 + dt**2/(48*mu*epsilon) * z10 + dt**4/(1920*mu**2*epsilon**2) * z12))
    ...
    z15 = solverM22.solve(M22_g, S12_g.T * z14)
    z16 = solverM11.solve(M11_g, S12_g * z15)
    w3 = 1/4*S12_g.T * (v2 + dt**2/(48*mu*epsilon) * z14 + dt**4/(1920*mu**2*epsilon**2) * z16) + mu/dt*M22_g * v3
...
S_LHS_precond_0 = sprs.bmat([[2/dt*M00_g, epsilon/4*(-S01_g + dt**2/48*X01_g_approx - dt**4/1920*Y01_g_approx), None], 
                                [-1/4*(S01_g.T - dt**2/48*X01_g_approx.T + dt**4/1920*Y01_g_approx.T), -2*epsilon/dt*M11_g, 
                                1/2*(S12_g + dt**2/(48*mu*epsilon)*X12_g_approx + dt**4/(1920*mu**2*epsilon**2)*Y12_g_approx)],
                                [None, 1/4*(S12_g.T + dt**2/(48*mu*epsilon)*X12_g_approx.T + 
                                dt**4/(1920*mu**2*epsilon**2)*Y12_g_approx.T), mu/dt*M22_g]], format='csr')
...
# S0_LHS = sprs.bmat([[2/dt*M00_g, epsilon/4*(-S01_g + dt**2/48*X01_g - dt**4/1920*Y01_g), None], 
#                     [1/4*(S01_g.T - dt**2/48*X01_g.T + dt**4/1920*Y01_g.T), 2*epsilon/dt*M11_g, 
#                     -1/2*(S12_g + dt**2/(48*mu*epsilon)*X12_g + dt**4/(1920*mu**2*epsilon**2)*Y12_g)],
#                     [None, 1/4*(S12_g.T + dt**2/(48*mu*epsilon)*X12_g.T + dt**4/(1920*mu**2*epsilon**2)*Y12_g.T), 
#                     mu/dt*M22_g]], format='csr')
...
# M0_precond = spla.LinearOperator((N0 + N1 + N2, N0 + N1 + N2), matvec=(spla.spilu(S_LHS, drop_tol = 1e-12)).solve)
...
for time_step in tqdm.tqdm(range(2, number_of_time_steps)):
    ...
    E3 = solverM00.solve(M00_g, S01_g * E2)
    E4 = solverM11.solve(M11_g, S01_g.T * E3)
    bp_RHS = 1/dt*M00_g * p[time_step - 1] + epsilon/2 * S01_g * (E[time_step - 1] - dt**2/12 * E2 + dt**4/120 * E4)
    ...
    H3 = solverM11.solve(M11_g, S12_g * H2)
    H4 = solverM22.solve(M22_g, S12_g.T * H3)
    bE_RHS = (1/2 * S01_g.T * (p[time_step - 1] - dt**2/12 * p2 + dt**4/120 * p4) - epsilon/dt * M11_g * E[time_step - 1] -
                1/2 * S12_g * (H[time_step - 1] + dt**2/(12*mu*epsilon) * H2 + dt**4/(120*mu**2*epsilon**2) * H4))

    E1 = solverM22.solve(M22_g, S12_g.T * E[0])
    E2 = solverM11.solve(M11_g, S12_g * E1)
    E3 = solverM22.solve(M22_g, S12_g.T * E2)
    E4 = solverM11.solve(M11_g, S12_g * E3)
    bH_RHS = (-1/2 * S12_g.T * (E[time_step - 1] + dt**2/(12*mu*epsilon) * E2 + dt**4/(120*mu**2*epsilon**2) * E4) + 
                    mu/dt*M22_g * H[time_step - 1])

    # Setup right hand side for intermediate variables p, E and H
    b_RHS = np.concatenate((bp_RHS, bE_RHS, bH_RHS))

#     # Setup right hand side for intermediate variables p, E and H
#     b_RHS_direct = sprs.bmat([[1/dt*M00_g, epsilon/2*(S01_g - dt**2/12*X01_g + dt**4/120*Y01_g), None], 
#                               [1/2*(-S01_g.T + dt**2/12*X01_g.T - dt**4/120*Y01_g.T), epsilon/dt*M11_g, 
#                                1/2*(S12_g + dt**2/(12*mu*epsilon)*X12_g + dt**4/(120*mu**2*epsilon**2)*Y12_g)],
#                               [None, -1/2*(S12_g.T + dt**2/(12*mu*epsilon)*X12_g.T + dt**4/(120*mu**2*epsilon**2)*Y12_g.T), 
#                                mu/dt*M22_g]], format='csr') * np.concatenate(
#                                                               (p[time_step - 1], E[time_step - 1], H[time_step - 1]))
    ...
    # M_precond = spla.LinearOperator((N0 + N1 + N2, N0 + N1 + N2), matvec=(spla.spilu(S_LHS, drop_tol = 1e-12)).solve)
